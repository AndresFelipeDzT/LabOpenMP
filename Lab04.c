...
*    al ser variables privadas (que no se comparten entre los hilos). En este
*    caso todas las variables son privadas evitando conflictos de un valor cambiante
*    entre iteraciones y ver un ejemplo de sincronizacion.
******************************************************/
...
int main() {
    ...
    //En este caso i, a y b son privadas para cada hilo
    #pragma omp parallel for private(i) private(a) private(b)
    for (i = 0; i < N; i++) {
        b = a + i;  // Suma dentro del hilo
    }

    //Se muestra el valor de a y b que ahora no varia entre iteraciones
